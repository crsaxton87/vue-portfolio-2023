<template>
  <div>
    <div
      ...
      :class="{ hidden: !cartVisible, 'translate-x-full': !cartVisible }"
    >
      <h3 class="absolute left-5 top-5">Cart</h3>
      <HeroIcons
        icon="x"
        class="absolute right-4 top-5 cursor-pointer hover:text-theme-r active:text-black"
        @click="emit('cart-toggle')"
      />
      <div
      ...
      >
        ...
        <div class="flex justify-end">
          <HeroIcons
            icon="trash"
            class="ml-2 cursor-pointer hover:text-theme-r active:text-black"
    ...
    <div
      class="absolute left-0 top-0 z-20 m-0 h-full w-full bg-white bg-opacity-50 p-0 backdrop-blur-sm"
      :class="{ hidden: !cartVisible }"
      @click="emit('cart-toggle')"
    ></div>
  </div>
</template>
